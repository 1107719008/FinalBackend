...
def db_feed(db: Session):
    new_rent_list = [DbRent(
        ...
        rentpicfour_url=rent["rentpicfour_url"],
        rentintro=rent["rentintro"],
...
def create(db: Session, request: RentRequestSchema) -> DbRent:
    new_rent = DbRent(
        ...
        rentpicfour_url=request.rentpicfour_url,
...
def get_rent_by_category(category: str, db: Session) -> list[DbRent]:
    # time.sleep(2)
    rent = db.query(DbRent).filter(func.upper(DbRent.rentcategory) == category.upper()).all()
    if not rent:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
