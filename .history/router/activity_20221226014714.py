...
from router.schemas import ActivityRequestSchema, ActivityResponseSchema,ActivityResponseWithUserSchema
...
def feed_initial_activities(db: Session = Depends(get_db)):
...
@router.get('/id/{product_id}', response_model=ActivityResponseWithUserSchema)
def get_product_by_id(product_id: int, db: Session = Depends(get_db)):
    return db_activity.get_product_by_id(product_id=product_id, db=db)